...
function Coinfo({isadmin,cus}) {
    ...
    const makedt = async ()=>{
        const res = await fetch(`${backend}/api/codt/${cus}`,{method:'GET'})
        const data = await res.json();
        if(data.status=="ok"){
            setUd({...ud,status:'Deliverd'})
        }else{
            alert(data.status);
        }
    }
  ...
  return (
    <div>
        <div style={{cursor:'pointer',margin:'10px 0',display:'flex',alignItems:'center',gap:'10px'}} onClick={()=>{nav("/corder")}}><BsArrowLeft style={{cursor:'pointer',scale:'1.5',marginLeft:'5vw'}} /> All Orders</div>
        <div>
            ...
            <div className='cospec'>
                ...
                <div className='pcodb' style={{ display:`${ud.status=="Undeliverd"?"flex":"none"}` }}>
                    <div className='codb' onClick={makedt}>Deliver</div>
                </div>
            </div>
        </div>
    </div>
